...
def _apply_direct_alias_endpoint(st: "_Switch", da: DirectAlias) -> None:
    ...
    # Preserve an explicit API mode from the alias's named provider. A generic
    # endpoint URL cannot identify custom transports such as CPA Gemini's
    # codex_responses mode, so re-detecting from the URL would route it to
    # chat_completions incorrectly.
    if isinstance(alias_runtime, dict) and alias_runtime.get("api_mode"):
        st.api_mode = alias_runtime["api_mode"]
