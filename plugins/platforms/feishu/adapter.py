...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def _build_event_handler(self) -> Any:
        ...
        menu_handler = getattr(self, "_on_bot_menu_event", None)
        if callable(register_bot_menu) and callable(menu_handler):
            builder = register_bot_menu(menu_handler)
        elif callable(register_bot_menu):
            logger.warning(
                "[Feishu] Bot-menu SDK hook is available but no menu handler is installed; "
                "the bot menu is disabled"
            )
        else:
            logger.warning(
    ...
    async def _send_interactive_card(
        self, chat_id: str, card: Dict[str, Any], metadata: Optional[Dict[str, Any]] = None,
        failure_message: str = "interactive card send failed", *,
        state_map: Optional[Dict[int, Dict[str, str]]] = None, state_id: Optional[int] = None,
        session_key: str = "",
    ) -> SendResult:
        ...
        if result.success and state_map is not None and state_id is not None:
            state_map[state_id] = {
