...
def _resolve_reasoning(model: str, params: dict[str, Any]) -> tuple[Any, bool]:
    ...
    if params.get("is_xai_responses", False):
        ...
        if declared is not None:
            ...
            # Gemini 3 model IDs use a narrower, model-specific thinkingLevel
            # vocabulary than the generic Codex ladder. CPA's Responses endpoint
            # translates this reasoning field to Gemini levels; do not leak
            # xhigh/max into strict Gemini 3.8/3.7 Flash requests.
            supported = gemini_supported_efforts(model) or codex_supported_efforts(model)
