...
def test_feishu_menu_settings_keep_only_slash_routes() -> None:
    settings = FeishuAdapter._load_settings(
        {
            "menu_default_chat_id": "oc_target",
            "menu_routes": {
                "panel": "/panel",
                "status": "/panel status",
    ...
    assert settings.menu_default_chat_id == "oc_target"
    assert settings.menu_routes == {"panel": "/panel", "status": "/panel status"}


def test_event_handler_registers_bot_menu_when_sdk_exposes_hook(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        type("FakeDispatcher", (), {"builder": staticmethod(lambda *_args: builder)}),
    )
    adapter = object.__new__(FeishuAdapter)
    setattr(adapter, "_encrypt_key", "")
    setattr(adapter, "_verification_token", "")

    result = adapter._build_event_handler()

    assert result is builder
    assert any(name == "register_p2_application_bot_menu_v6" for name, _args in calls)


@pytest.mark.asyncio
async def test_bot_menu_event_routes_to_configured_command() -> None:
    adapter = object.__new__(FeishuAdapter)
    setattr(adapter, "_menu_routes", {"panel": "/panel"})
    setattr(adapter, "_menu_default_chat_id", "oc_target")
    setattr(adapter, "_is_interactive_operator_authorized", lambda open_id: True)
    route = AsyncMock()
    setattr(adapter, "_route_menu_command", route)
    data = SimpleNamespace(
        event=SimpleNamespace(
            event_key="panel",
            operator=SimpleNamespace(operator_id=SimpleNamespace(open_id="ou_owner")),
        )
    )
