...
面板显示实际生效的会话或频道路由，并读取飞书专属的推理显示设置。重复选择完全相同的设置会保留当前 Agent；凭据或能力更新仍会生效。保存期间，新回合会提示稍后重试，新建/恢复会话会等待保存完成；关闭面板不会取消已经开始的保存。
...
请先按自己的供应商定义这些模型别名。供应商可见性由当前 Profile 配置决定；相同 Profile 和相关供应商/授权配置的面板共享模型发现任务，各面板单独计算当前选项。目录结果缓存五分钟；点击刷新会重新获取供应商库存，同时刷新底层模型缓存。新建/恢复会话后，之前启动的会话和状态查询不再回填页面。
...
每次卡片操作的总预算为五秒，包括鉴权、话题查询和重试。回合结束时最多再等待五秒完成最终进度更新，随后停止发布卡片状态并释放会话。
